...
def optimize_model(Qp, Rq, Pq, Vp, d, Sp, X, Y, gamma):
    ...
    beta = pulp.LpVariable.dicts("beta", [(i, k) for i in candidates for k in rival_plans + ['r11']], lowBound=0, cat='Continuous')
    ...
    for i in candidates:
        model += pulp.lpSum([alpha[i, j] * (X[i, j] - Qp[j] * (Vp[j] * gamma.get((i, j), 0) + Sp[j])) for j in our_plans]) + \
                 pulp.lpSum([beta[i, k] * (Y[i, k] - Rq[k] * Pq[k]) for k in rival_plans + ['r11']]) <= z

    for i in candidates:
        model += pulp.lpSum([alpha[i, j] for j in our_plans]) + pulp.lpSum([beta[i, k] for k in rival_plans + ['r11']]) == d[i]

    # Solve the model
    model.solve()

    return {i: pulp.value(pulp.lpSum([alpha[i, j] for j in our_plans])) for i in candidates}
...
rival_salary_values = np.linspace(30.0, 20.0, 10)
selected_candidates_quality = []
selected_candidates_salary = []
...
for quality in rival_quality_values:
    ...
    new_Y = Y.copy()
    for i in candidates:
        new_Y[i, 'r11'] = Y[i, 'r1']  # Assume the same required salary for simplicity
    selection = optimize_model(Qp, Rq_new, Pq_new, Vp, d, Sp, X, new_Y, gamma)
    selected_candidates_quality.append(sum(selection.values()))
...
for salary in rival_salary_values:
    ...
    for i in candidates:
        new_Y[i, 'r11'] = Y[i, 'r1']  # Assume the same required salary for simplicity
    selection = optimize_model(Qp, Rq_new, Pq_new, Vp, d, Sp, X, new_Y, gamma)
    selected_candidates_salary.append(sum(selection.values()))

# Plotting the sensitivity analysis
plt.figure(figsize=(12, 6))

plt.subplot(1, 2, 1)
plt.plot(rival_quality_values, selected_candidates_quality, marker='o')
plt.xlabel('New Rival Plan Quality')
plt.ylabel('Number of Candidates Selected Our Plans')
plt.title('Sensitivity Analysis for New Rival Plan Quality (Model 2)')

plt.subplot(1, 2, 2)
plt.plot(rival_salary_values, selected_candidates_salary, marker='o')
plt.xlabel('New Rival Plan Salary')
plt.ylabel('Number of Candidates Selected Our Plans')
plt.title('Sensitivity Analysis for New Rival Plan Salary (Model 2)')
