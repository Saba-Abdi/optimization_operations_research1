...
* 41 points from -20% to +20% in steps of 1%
Set
    sens /sens1*sens41/;

Parameter
    sens_val(sens) /sens1 -0.2, sens2 -0.19, sens3 -0.18, sens4 -0.17, sens5 -0.16,
                    sens6 -0.15, sens7 -0.14, sens8 -0.13, sens9 -0.12, sens10 -0.11,
                    sens11 -0.1, sens12 -0.09, sens13 -0.08, sens14 -0.07, sens15 -0.06,
                    sens16 -0.05, sens17 -0.04, sens18 -0.03, sens19 -0.02, sens20 -0.01,
                    sens21 0, sens22 0.01, sens23 0.02, sens24 0.03, sens25 0.04,
                    sens26 0.05, sens27 0.06, sens28 0.07, sens29 0.08, sens30 0.09,
                    sens31 0.1, sens32 0.11, sens33 0.12, sens34 0.13, sens35 0.14,
                    sens36 0.15, sens37 0.16, sens38 0.17, sens39 0.18, sens40 0.19,
                    sens41 0.2 /;

Parameter
    results(sens) objective function value for each sensitivity analysis
    alpha_results(i,j,sens) alpha values for each sensitivity analysis
    beta_results(i,k,sens) beta values for each sensitivity analysis
;

Loop(sens,
    Qp('p3') = 0.8 * (1 + sens_val(sens));
    Qp('p4') = 1 * (1 + sens_val(sens));
    Qp('p5') = 0.8 * (1 + sens_val(sens));
    Qp('p6') = 0.7 * (1 + sens_val(sens));

    solve bi_level using LP minimizing z;

    results(sens) = z.l;
    alpha_results(i,j,sens) = alpha.l(i,j);
    beta_results(i,k,sens) = beta.l(i,k);
);

display results, alpha_results, beta_results;
