...
* Sensitivity Analysis for Mp(j)
Parameter Mp_min(j), Mp_max(j), initial_optimal_plans(j);
Scalar step / 0.1 /;

loop(j,
    initial_optimal_plans(j) = Op.l(j);
    if(Op.l(j) > 0.5,
        initial_optimal_plans(j) = yes;
    else
        initial_optimal_plans(j) = no;
    );
);

* Declare the scalars outside the loop
Scalar changed_down, changed_up;

* Declare the model outside the loop
model bi_level /all/;

* Find the upper bound for Mp(j)
loop(j,
    Mp_max(j) = Mp(j);
    changed_up = 0;
    while(changed_up = 0,
        Mp(j) = Mp_max(j) + step;
        solve bi_level using MIP maximizing z;
        if (abs(Op.l(j) - initial_optimal_plans(j)) > 0.001,
            changed_up = 1;
            Mp(j) = Mp_max(j);
        else
            Mp_max(j) = Mp(j);
...
* Reset Mp(j) to initial values
loop(j,
    Mp(j) = Mp(j) - step * (card(j))
);

* Find the lower bound for Mp(j)
loop(j,
    Mp_min(j) = Mp(j);
    changed_down = 0;
    while(changed_down = 0,
        Mp(j) = Mp_min(j) - step;
        solve bi_level using MIP maximizing z;
        if (abs(Op.l(j) - initial_optimal_plans(j)) > 0.001,
            changed_down = 1;
